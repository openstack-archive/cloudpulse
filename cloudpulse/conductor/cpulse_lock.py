...
class CpulseLock(object):
    ...
    def acquire(self, retry=True):
        lock_conductor_id = objects.CpulseLock.create(self.cpulse_test.uuid,
                                                      self.conductor_id)
        if lock_conductor_id is None:
            return
        else:
            raise exception.TestLocked(uuid=self.cpulse_test.uuid)

    def release(self, test_uuid):
        result = objects.CpulseLock.release(test_uuid, self.conductor_id)
        if result is True:
            LOG.debug("Lock was already released on test %s!" % test_uuid)
        else:
...
def thread_lock(cpulse_test, conductor_id):
    ...
    try:
        ...
        with excutils.save_and_reraise_exception():
            cpulselock.release(cpulse_test.uuid)
    finally:
        cpulselock.release(cpulse_test.uuid)
