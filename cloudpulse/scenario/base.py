...
class Scenario(object):
    ...
    """
    def __init__(self, context=None, admin_tests=None, tenant_tests=None, operator_tests=None):
    ...
    def list_operator_scenarios(scenario_cls):
        ...
        scenarios_list = [
            ...
            for func in dir(scenario) if Scenario.is_scenario(scenario, func) and Scenario.is_operator(scenario, func)]
    ...
    def list_admin_scenarios(scenario_cls):
        ...
        scenarios_list = [
            ["%s.%s" % (scenario.__name__, func)
            for func in dir(scenario) if Scenario.is_scenario(scenario, func) and Scenario.is_admin(scenario, func)]
            for scenario in scenario_classes
        ]
    ...
    def list_tenant_scenarios(scenario_cls):
        ...
        scenarios_list = [
            ["%s.%s" % (scenario.__name__, func)
            for func in dir(scenario) if Scenario.is_scenario(scenario, func) and 
    ...
    def validate(cls, name, config, admin=None, users=None, task=None):
        ...
        if self.meta(name, "admin_only") == True:
            print("Admin Access")
        if self.meta(name, "operator") == True:
            print("Operator")
