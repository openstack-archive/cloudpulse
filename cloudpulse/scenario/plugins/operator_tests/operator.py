...
def execute(command):
    """ Execute a command (string) and returns the status, output or an exception.
    """
    try:
...
class operator_scenario(base.Scenario):
    ...
    def galera_check(self):
        ...
        if not res['status']:
            ...
            if not 'wsrep_incoming_addresses' in galera_status:
    ...
    def node_check(self):
        ...
        anscmd = "ansible -o all  -i '%s' -m ping -u root" % ','.join(nova_hypervisor_list[2])
        res = execute(anscmd)
        res['output'] = res['output'].split('\n')
        output = filter(lambda x: not re.match(r'^\s*$', x), res['output'])
        for line in output:
